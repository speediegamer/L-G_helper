...
# Create a OpenRC service to initialize the GPU on startup

cp lg-service-openrc /etc/init.d/lg-service-openrc
chmod 644 /etc/init.d/lg-service-openrc

mv lg_start.sh /usr/bin/lg_start.sh
chmod +x /usr/bin/lg_start.sh

chmod +x /etc/init.d/lg-service-openrc

rc-update add /etc/init.d/lg-service-openrc default
rc-service /etc/init.d/lg-service-openrc start
